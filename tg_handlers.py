...
async def start_command(update, context):
    ...
    Используйте /catalog или кнопку ниже чтобы начать покупки или кнопки!
    Или /help для списка всех команд.
    Или /contact для ответа живых людей.
    
    Данный раздел можно доработать по вашему пожеланию.
    """
...
async def about_callback(update, context):
    ...
    • Бот подключается к БД через специального пользователя 
    • Назначение прав к таблицам, функциям
